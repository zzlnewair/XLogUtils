package com.zzl.logutils.log.crash;
...
import com.zzl.logutils.log.log.util.AppUtils;
import com.zzl.logutils.log.log.util.FileUtil;
import com.zzl.logutils.log.log.util.TimeUtils;
...
public class ThreadCatchInterceptor {
    ...
    private List<String> crashInfo = new ArrayList<String>();

    // Log Config
    private LogConfig logConfig = null;

    // Crash Save Directory
    private String PATH = Environment.getExternalStorageDirectory().getAbsolutePath()  + "/CrashLog/";
    ...
    public void install(final CallBack callBack) {
        if (!FileUtil.existSDCard()) {
            ...
            PATH = logConfig.getContext().getFilesDir().getAbsolutePath()+ "/";
            logger.log("path=="+PATH);
        
        } else {
            if (!FileUtil.createDirectory(PATH)) {
                logger.log("Create XLog directory fails, which causes the crash information could not be saved.");
            }
        }
    ...
    private boolean handleException(Throwable throwable) {
        ...
        if (logConfig.getContext() != null) {
            crashInfo.add("Version:" + logConfig.getContext().getPackageName()+AppUtils.getVersionName(logConfig.getContext()));
        }
    ...
    private String save(Throwable throwable) {
        ...
        String savePath = PATH + TimeUtils.formatTime()  + ".log";
        try {
